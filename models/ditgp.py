...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.dense = nn.Linear(hidden_size, 2 * in_channels * hidden_size)
        # bits ---> hidden_size
    ...
    def forward(self, x, t, bert_features, grid_masks):
        ...
        outputs = self.dense(bert_features)
        ...
        pos_emb = self._sinusoidal_position_embedding(b, l, self.hidden_size, x.device)
        ...
        conditions = torch.einsum('bmhd,bnhd->bhmn', qw, kw)
        # [bsz, len, len, bits]
        ...
        t = self.time_mlp(t).unsqueeze(dim=1).expand(-1, x.shape[1], -1)  # [bsz, 1, hidden_size]

        for block in self.blocks:
            x = block(x, t)  # (N, T, D)
        # [bsz, len, len ,bits]
        x = self.final_layer(x, t)
        x = self.unpatchify(x).permute(0, 2, 3, 1)
        return x
