...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.down_linear = nn.Linear(hidden_size * 2, hidden_size)
    ...
    def forward(self, x, t, bert_features, attention_mask, x_self_cond=None):
        ...
        if x_self_cond is None:
            x_cond = torch.zeros_like(x_emb)
        else:
            x_cond = self.x_embedder(x_self_cond)
        x = self.down_linear(torch.cat([x_emb, x_cond], dim=-1)) + self.pos_embed(pos_ids)
        # x = self.x_embedder(x) + self.pos_embed(pos_ids) # [bsz, len, hidden_size]
        t = self.time_mlp(t).unsqueeze(dim=1).expand(-1, x.shape[1], -1) # [bsz, len, hidden_size]
