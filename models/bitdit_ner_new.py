...
from data.ner_dataset import LabelSetNEW
from .ditnew import DiT
from einops import repeat
from .unet import Unet
from .ditnew import Biaffine
...
class DiffusionEntityRecognizer(nn.Module):
    def __init__(self,
                 ...
                 patch_size: int = 4,
                 max_length: int = 64,
                 network_architecture: str = 'transformer'):
        super().__init__()
        ...
        self.biaffine = Biaffine(n_in=dim_model, n_out=dim_model)
        ...
        self.label_set = LabelSetNEW(dataset)
        self.bits = torch.ceil(torch.log2(torch.tensor(len(self.label_set)))).long()
        self.loss_type = loss_type
        # self.predictor = DiffusionPredictor(dim_model, 16, self.bits.item())

        if network_architecture == 'transformer':
            ...
            self.predictor = Unet(dim=64, channels=self.bits.item(), condition_dim=dim_model)
        else:
    ...
    @torch.no_grad()
    def ddim_sample(self, features, grid_masks, attention_mask, shape, time_difference=0):
        ...
        grids = torch.randn(shape, device=device)
        ...
        for times, times_next in time_pairs:
            ...
            padded_log_snr, padded_log_snr_next = map(partial(right_pad_dims_to, grids), (log_snr, log_snr_next))
            ...
            # predict x0
            x_start = self.predictor(grids, log_snr, features, grid_masks, attention_mask)
            ...
            pred_noise = (grids - alpha * x_start) / sigma.clamp(min=1e-8)

            # calculate x next

            grids = x_start * alpha_next + pred_noise * sigma_next

        return bits_to_decimal(grids, self.bits.item())
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor,
                grid_labels: torch.tensor, grid_masks: torch.tensor):
        """
        ...
        Args:
            ...
            grid_labels: [bsz, word_len, word_len]
            grid_masks: [bsz, word_len, word_len]
        ...
        if not self.training:
            shape = (bsz, features.shape[1], features.shape[1], self.bits.item())
            results = self.ddim_sample(features, grid_masks, attention_mask, shape)
            return results

        if self.training:
            times = torch.zeros((bsz,), device=self.device).float().uniform_(0, 1.)
            # categorical data to bits [bsz, ]
            bits_grids = decimal_to_bits(grid_labels, self.bits)
            bits_grids *= self.scale
            noise = torch.randn_like(bits_grids)

            noise_level = self.log_snr(times)
            padded_noise_level = right_pad_dims_to(bits_grids, noise_level)
            alpha, sigma = log_snr_to_alpha_sigma(padded_noise_level)

            noised_grids = alpha * bits_grids + sigma * noise

            # self_cond = None
            # if random() < 0.5:
            #     with torch.no_grad();
            #         self_cond = self.predictor(noised_grids, noise_level)
            # noised_grids = torch.masked_fill(noised_grids, grid_masks.eq(0).unsqueeze(dim=-1), 0.0)
            pred = self.predictor(noised_grids, noise_level, features, grid_masks, attention_mask)

            targets = bits_grids
            targets_mask = grid_masks.unsqueeze(dim=-1).expand(-1, -1, -1, self.bits)
            if self.objective == 'pred_noise':
                targets = noise
