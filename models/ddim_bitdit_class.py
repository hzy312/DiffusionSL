...
class BitDit(nn.Module):
    def __init__(self,
        ...
        self.class_embedding = nn.Embedding(self.num_classes, self.bits)

        self.loss_type = loss_type
        self.model = DiT(in_channels=self.bits,
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor, seq_labels: torch.tensor,
        ...
        if not self.training:
            shape = (*features.shape[:2], self.bits)
            results = self.sample(shape, features, label_mask)
            bit_seq, path_x = results
            path_x = torch.stack([self.emb_to_label(r) for r in path_x], dim=1)
            results = self.emb_to_label(bit_seq)
        ...
        if self.training:
            ...
            targets_mask = label_mask.unsqueeze(dim=-1).expand(-1, -1, self.bits)
            if self.objective == 'pred_noise':
                targets = noise
