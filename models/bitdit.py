...
class BitDit(nn.Module):
    def __init__(self,
        ...
        # self.class_embedding = nn.Embedding(self.num_classes, self.dim_model)
    ...
    def emb_to_label(self, embedding: torch.tensor):
        """
        calculate the cosine similarity of predicted embedding and class embedding
        Args:
            embedding: [bsz, len, dim]
            self.class_embedding: [num_classes, dim]
        Returns:
            results: [bsz, len, num_classes] ---> [bsz, len]
        """
        embedding = embedding.unsqueeze(dim=2)
        cls_embedding = self.class_embedding.weight.unsqueeze(dim=0).unsqueeze(dim=0)
        simi = torch.cosine_similarity(embedding, cls_embedding, dim=-1)
        label = torch.argmax(simi, dim=-1)
        return label

    @torch.no_grad()
    def ddpm_sample(self, features, attention_mask, shape, ensemble=False, time_difference=0):
        ...
        for time, time_next in time_pairs:
            ...
            x_start = self.predictor(ner_labels, log_snr, features, attention_mask, x_start)
            ...
            log_snr, log_snr_next = map(partial(right_pad_dims_to, ner_labels), (log_snr, log_snr_next))
            ...
            mean = alpha_next * (ner_labels * (1 - c) / alpha + c * x_start)
            ...
            noise = torch.where(
                ...
                torch.randn_like(ner_labels),
                torch.zeros_like(ner_labels)
            )

            ner_labels = mean + (0.5 * log_variance).exp() * noise

        # return self.emb_to_label(x_start)
        return bits_to_decimal(x_start, self.bits.item())

    @torch.no_grad()
    def ddim_sample(self, features, attention_mask, shape, ensemble=False, time_difference=0):
        batch, device = shape[0], self.device
        ...
        for times, times_next in time_pairs:
            ...
            if ensemble:
                results_per_step.append(ner_labels)

        if ensemble:
            # [bsz, len, num_step]
            results = torch.stack(results_per_step, dim=-1).mean(dim=-1)
            return bits_to_decimal(results, self.bits.item())
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor, seq_labels: torch.tensor,
        ...
        if not self.training:
            ...
            results = self.ddpm_sample(features, (1 - label_mask.long()), shape, ensemble)
            return results
        ...
        if self.training:
            ...
            # seq_labels[label_mask] = 0
            # bits_seq_labels = self.class_embedding(seq_labels)
            ...
            if random() < 0.5:
                with torch.no_grad():
                    self_cond = self.predictor(noised_seqs, noise_level, features, (1 - label_mask.long()))

            pred = self.predictor(noised_seqs, noise_level, features, (1 - label_mask.long()), self_cond)

            targets = bits_seq_labels
            targets_mask = (1 - label_mask.long()).unsqueeze(dim=-1).expand(-1, -1, self.bits.item())
            if self.objective == 'pred_noise':
                targets = noise
