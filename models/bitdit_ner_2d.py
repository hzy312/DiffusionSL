...
from data import LabelSet2D
from .dit2D import DiT
...
class DiffusionEntityRecognizer(nn.Module):
    def __init__(self,
        ...
        if self.add_lstm:
            self.lstm = nn.LSTM(dim_model, dim_model // 2, num_layers=1, batch_first=True, bidirectional=True)
        ...
        self.label_set = LabelSet2D(dataset)
    ...
    @torch.no_grad()
    def ddim_sample(self, features, grid_masks, shape, time_difference=0):
        ...
        for times, times_next in time_pairs:
            ...
            x_start = self.predictor(grids, log_snr, features, grid_masks)
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor,
                words2pieces: torch.tensor, grid_labels: torch.tensor, grid_masks: torch.tensor, lengths):
        """
        ...
        if not self.training:
            ...
            results = self.ddim_sample(features, grid_masks, shape)
            return results
        ...
        if self.training:
            ...
            pred = self.predictor(noised_grids, noise_level, features, grid_masks)
