...
class DiT(nn.Module):
    ...
    def forward(self, x, t, bert_features, grid_masks):
        """
        Forward pass of DiT.
        x: [bsz, len, len, bits]
        ...
        pos_ids = torch.arange(x_emb.shape[1], dtype=torch.long, device=x.device).unsqueeze(dim=0)
        x = x_emb + self.pos_embed(pos_ids)  # [bsz, len, hidden_size]
        t = self.time_mlp(t).unsqueeze(dim=1)  # [bsz, 1, hidden_size]
        pre, post = bert_features.unsqueeze(dim=2).expand(-1, -1, bert_features.shape[1], -1),\
                    bert_features.clone().unsqueeze(dim=1).expand(-1, bert_features.shape[1], -1, -1)
        grid_features = self.fuse_linear(torch.cat([pre, post], dim=-1))
        # [bsz, len, len, hidden_size]
        grid_features = torch.masked_fill(grid_features, grid_masks.eq(0).unsqueeze(dim=-1), 0)
        grid_features = self._fill(grid_features)
        # [bsz, n, hidden_size]
        grid_features_patch = self.grid_embedder(grid_features)
        c = t + grid_features_patch  # [bsz, len, hidden_size]
        for block in self.blocks:
            x = block(x, c)  # (N, T, D)
        x = self.final_layer(x, c)
        # [bsz, len, len ,bits]
        x = self.unpatchify(x).permute(0, 2, 3, 1)[:, :l, :l, :]
        return x

    def _fill(self, x: torch.tensor):
        b, l, _, in_cha = x.shape
        if l % self.patch_size != 0:
            remain = l % self.patch_size
            temp = torch.zeros(size=[b, l + (self.patch_size - remain), l + (self.patch_size - remain), in_cha], dtype=x.dtype, device=x.device)
            temp[:, :l, :l, :] = x
            x = temp
        return x
