...
from data import LabelSetGP
from .ditgp import DiT
...
class DiffusionEntityRecognizer(nn.Module):
    def __init__(self,
        ...
        self.predictor = DiT(in_channels=len(self.label_set),
    ...
    #             c * pred_noise + \
    #             sigma * noiseself.bits
    #     x_decimal = bits_to_decimal(x, )
    #     # [bsz, len]
    #     return x_decimal
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor,
        ...
        if not self.training:
            shape = (bsz, features.shape[1], features.shape[1], len(self.label_set))
        ...
        if self.training:
            ...
            bits_grids = (grid_labels * 2 - 1).float()
            ...
            targets = bits_grids
            targets_mask = grid_masks.unsqueeze(dim=-1).expand(-1, -1, -1, len(self.label_set))
            if self.objective == 'pred_noise':
                targets = noise
